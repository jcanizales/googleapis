...
# TODO(jcanizales): Abort if the input has a "visibility" section. It needs to
# be applied manually to the input and then removed.
...
# Have ruamel write to a string, because it can produce multiple newlines in a
# row, and trailing whitespace.
output = StringIO()
...
ruamel.yaml.round_trip_dump(dom, output)

# Remove multiple newlines in a row and trailing whitespace.
